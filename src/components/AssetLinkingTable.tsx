    ...
    <div className="space-y-6">
      ...
      <div className="mb-4">
        <div className="flex items-center mb-2">
          ...
          />
          <label htmlFor="transactAsWhole" className="font-medium">Transact as a whole</label>
        </div>
      ...
      <div className="overflow-x-auto bg-yellow-50 rounded-md">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-yellow-100">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider"></th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Relation</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Asset Tag ID</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            ...
            <tr className="bg-blue-50">
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">this asset &gt;</td>
            ...
            {isChildAsset && validParentLinks.map((link) => (
              <tr key={link.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Parent</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{link.fromAsset.serialNumber}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{link.fromAsset.name}</td>
            ...
            {!isChildAsset && validChildLinks.map((link) => (
              <tr key={link.id}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"></td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Child</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{link.toAsset.serialNumber}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{link.toAsset.name}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm">
                  <UnlinkAssetButton
                    assetId={asset.id}
                    linkId={link.id}
