...
export default function ScheduleDetailPage({
  ...
  const fetchSchedule = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/maintenance-schedules/${resolvedParams.id}`
      );
  ...
  const generateTask = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "/api/maintenance-schedules/generate-tasks",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        if (result.generatedTasks > 0) {
          alert(
            `Successfully generated ${result.generatedTasks} maintenance task!`
          );
          fetchSchedule(); // Refresh to show the new task
        } else {
          alert(
            "No new task was generated. A task may already exist or the schedule is not due yet."
          );
        }
      } else {
  ...
  const deleteSchedule = async () => {
    if (
      !confirm(
        "Are you sure you want to delete this maintenance schedule? This will also delete all associated tasks. This action cannot be undone."
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json().catch(() => ({}));
        alert(
          `Failed to delete schedule: ${errorData.error || "Unknown error"}`
        );
      }
    } catch (error) {
      ...
      >
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-4">
            The maintenance schedule you are looking for does not exist.
          </p>
          <button
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        {canManageSchedule() && (
          <div className="flex gap-2">
            ...
            <button
              onClick={() =>
                router.push(`/maintenance/scheduled/${schedule.id}/edit`)
              }
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 text-gray-300">
                  {schedule.leadTimeDays} days
                </span>
              </div>
              <div>
                <span className="text-gray-400">Auto Assign:</span>
                <span className="ml-2 text-gray-300">
                  {schedule.autoAssign ? "Yes" : "No"}
                </span>
              </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500">
                      {schedule.template.maintenanceType}
                    </p>
                  </div>
                </div>
              )}
